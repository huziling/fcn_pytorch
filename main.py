...
parser.add_argument('--pathFig', type=str, default='../Auxiliary/figures/', help='path to figures')
parser.add_argument('--listFig', type=str, default='./Configure/figures.txt', help='path to figures list')
parser.add_argument('--listChart', type=str, default='./Configure/figures.txt', help='path to chart list')
parser.add_argument('--pathTable', type=str, default='../Auxiliary/tables/', help='path to tables')
parser.add_argument('--listTable', type=str, default='./Configure/tables.txt',  help='path to tables list')
parser.add_argument('--pathText', type=str, default='../Auxiliary/text/', help='path to text')
parser.add_argument('--pathBigText', type=str, default='./Configure/BigText.txt', help='path to text')
parser.add_argument('--listText', type=str, default='./Configure/text.txt', help='path to text list')
parser.add_argument('--pathCaption', type=str, default='../Auxiliary/figures/annotations/coco_raw.json',help='path to caption')
...
parser.add_argument('--imgWidthMinRatio', type=float, default=0.8, help='min img width (ratio\\textwidth)')
parser.add_argument('--imgWidthMaxRatio', type=float, default=1.0, help='max img width (ratio\\textwidth)')
parser.add_argument('--imgHeightMaxRatio', type=float, default=0.8, help='max img height (ratio\\textwidth)')
...
parser.add_argument('--distribution',
                    default={'figure': 0.2, 'table': 0.4, 'section': 0.5, 'list': 0.6, 'text': 0.98, 'line': 0.2},
...
parser.add_argument('--colors', default=['black', 'red', 'blue', 'cyan', 'magenta'], help='colors supported in latex')
parser.add_argument('--hasColors', type=float, default=0.4, help='probability to have color')
...
parser.add_argument('--proSigColumns', type=float, default=0.6, help='probability that a figure has no caption')
parser.add_argument('--proBigText', type=float, default=0.4, help='probability that a figure has no caption')

parser.add_argument('--maxNumElements', type=int, default=60, help='prob of having border line')

args = parser.parse_args()


# lists
fig_list = np.genfromtxt(args.listFig, delimiter='*', dtype=None)
chart_list = np.genfromtxt(args.listChart, delimiter='*', dtype=None)
table_list = np.genfromtxt(args.listTable, delimiter='*', dtype=None)
text_list = np.genfromtxt(args.listText, delimiter='*', dtype=None)
BigText_list= np.genfromtxt(args.pathBigText, delimiter='*', dtype=None)
...
for i in range(len(caption_list)):
    if caption_list[i]['file_path'][0:5]=='train':
        Temp.append(caption_list[i])
caption_list=Temp
...
for it_page in range(args.pageNum):
    ...
    try:
        ...
        if np.random.random()  < args.proSigColumns:
            num_columns=1
            ...
            while offset_y < (args.pageHeight - marginPt):
                ...
                if opt < args.distribution['figure']:
                    ...
                    if np.random.random() > args.figChart:
                        ...
                        with open(args.pathText + textfile.decode()) as f:
                            text = f.read()
                        ...
                        # img path
                        img_path = args.pathFig + np.random.choice(chart_list).decode()
                    ...
                    # img path
                    img_path = args.pathTable + np.random.choice(table_list).decode()
                    ...
                    if np.random.random() < args.proBigText:
                        # print("BigText")
                        img_path = args.pathText + np.random.choice(BigText_list).decode()
                        ...
                        # groundtruth
                        utils.update_groundtruth(groundtruth, groundtruth_element_count,
                                                 'section', img_path, '', [offset_y, offset_y + img_height,
                                                                         offset_x, offset_x + img_width])
                        # adjust
                        offset_x = marginPt + it_column * column_width + it_column * margin_delta
                        offset_y += img_height + font_size * args.paraSkipRatio
                    ...
                    item_sep = np.random.randint(30,70)*0.01
